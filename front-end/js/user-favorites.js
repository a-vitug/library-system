  ...
  books.forEach(book => {
    ...
    div.innerHTML = `
      <div class="favorite-card-header">
        ${
          book.thumbnail
            ? `<img src="${book.thumbnail}" class="favorite-cover">`
      ...
      <div class="favorite-card-body">
        ...
        <p class="favorite-author">${book.author || 'Unknown'}</p>
        ...
        <div class="favorite-actions">
          <button class="action-btn borrow-btn" onclick="openModal('${book._id}')">
            View
          </button>
        </div>
      </div>
...
async function openModal(bookId) {
  ...
  const books = await res.json();
  selectedBook = books.find(b => b._id === bookId);

  if (!selectedBook) return;

  document.getElementById('modalCover').src = selectedBook.thumbnail || '';
  document.getElementById('modalTitle').textContent = selectedBook.title;
  document.getElementById('modalAuthor').textContent = selectedBook.author || 'Unknown';
  document.getElementById('modalGenre').textContent = (selectedBook.genre || []).join(', ');
  ...
  if (selectedBook.available) {
    availability.innerHTML = `<span class="status-badge status-available">Available</span>`;
  } else {
    availability.innerHTML = `<span class="status-badge status-checked-out">Checked Out</span>`;
  }

  document.getElementById('modalPreview').href = selectedBook.previewLink || '#';
...
async function removeFavorite() {
  ...
  await fetch(`/api/user/favorites/${selectedBookId}`, {
    method: 'DELETE',
    headers: {
      Authorization: `Bearer ${token}`
    }
...
async function borrowBook() {
  ...
  const res = await fetch(`/books/checkout/${selectedBookId}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`
    }
